...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
        ...
        with tf.variable_scope('LPO_model', reuse=tf.AUTO_REUSE):
        ...
        self.C = C = tf.placeholder(tf.float32, [None])
        self.R_l = R_l = tf.placeholder(tf.float32, [None])
        self.v_l = v_l = tf.placeholder(tf.float32, [None])
        log_labda = tf.get_variable('LPO_model/Labda', None, tf.float32, initializer=tf.log(init_labda))
        ...
        # lf_loss = tf.reduce_mean(lf_losses1)
        ...
        self.l_lambda = tf.reduce_mean(ratio * tf.stop_gradient(lpred_) - tf.stop_gradient(lpred)+self.ALPHA3*self.C)
        ...
        # Final PG loss
        pg_loss = self.l_lambda*tf.stop_gradient(self.labda) - tf.stop_gradient(self.l_lambda) * log_labda
        # pg_loss = tf.reduce_mean(tf.maximum(pg_losses, pg_losses2)+ self.l_lambda * self.labda)
        approxkl = .5 * tf.reduce_mean(tf.square(neglogpac - OLDNEGLOGPAC))
        clipfrac = tf.reduce_mean(tf.to_float(tf.greater(tf.abs(ratio - 1.0), CLIPRANGE)))

        # Total loss
        loss = pg_loss - entropy * ent_coef + lf_loss * lf_coef

        # UPDATE THE PARAMETERS USING LOSS
        # 1. Get the model parameters
        params = tf.trainable_variables('LPO_model')
        # 2. Build our trainer
        if MPI is not None:
        ...
        self.loss_names = ['policy_loss', 'lyapunov_loss','policy_entropy', 'approxkl', 'clipfrac', 'lyapunov_lambda', 'alpha3']
        self.stats_list = [pg_loss, lf_loss,entropy, approxkl, clipfrac, self.labda, self.ALPHA3]
    ...
    def train(self, lr, cliprange, obs, obs_, returns, rewards, costs, masks, actions, values, neglogpacs, states=None):
        ...
        td_map = {
            ...
            self.CLIPRANGE : cliprange,
            self.OLDNEGLOGPAC : neglogpacs,
            self.OLDLPRED: values,
            self.ALPHA3: self.alpha3
        }
        if self.approximate_value_function is True:
            td_map.update({self.v_l: returns})
        else:
            td_map.update({self.v_l: costs})
