...
def learn(*, network, env, total_timesteps,n_of_paths=10, eval_env = None, seed=None, nsteps=2048, ent_coef=0.1, lr=3e-4,
            vf_coef=0.5,lf_coef=0.5, max_grad_norm=0.5, gamma=0.99, lam=0., alpha3 = .1, init_labda = 1.,
            log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2, use_adaptive_alpha3=False,approximate_value_function= True,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        obs, obs_, returns, rewards, costs, masks, actions, values, neglogpacs, states, epinfos = runner.run() #pylint: disable=E0632
        ...
        if states is None: # nonrecurrent version
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, nbatch, nbatch_train):
                    ...
                    slices = (arr[mbinds] for arr in (obs, obs_, returns, rewards, costs, masks, actions, values, neglogpacs,))
                    # print(**slices)
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, nenvs, envsperbatch):
                    ...
                    slices = (arr[mbflatinds] for arr in (obs,obs_, returns, rewards, costs, masks, actions, values, neglogpacs,))
        ...
        if model.use_adaptive_alpha3:
            if ep_L_R > ep_L_R_threshold:
                ...
                model.alpha3 = min(model.alpha3 * 1.5, 0.1)

            # model.ALPHA = min(model.ALPHA * 1.001, 0.1)
            model.alpha3 = min(model.alpha3 * 1.01, 0.1)
