...
class CPO(object):
    def __init__(self, a_dim, s_dim, args,
        ...
        #Lyapunov
        self.ladvantage = self.target_of_l - self.l
    ...
    def update(self, s, s_, returns, l_returns, advs, l_advs, a, old_values, old_l_values, l_reward, safety_eval,cliprangenow, LR_C, LR_L, length_of_trajectory):
        ...
        if self.use_lyapunov:
            ...
            feed_dict.update({self.target_of_l: l_returns, self.constraint_adv: l_advs})
            sub_feed_dict.update({self.target_of_l: sub_l_returns, self.constraint_adv: sub_l_advs})
...
def train(variant):
    ...
    for j in range(int(np.ceil(max_global_steps / sample_steps))):
        ...
        for _ in range(sample_steps):
            ...
            [action], [value], [l_value] = policy.choose_action(s)
            ...
            mb_obs_.append(s_)
            mb_rewards.append(r)
            mb_l_rewards.append(info['l_rewards'])
            ...
            current_path['l_rewards'].append(info['l_rewards'])
            current_path['violation'].append(info['violation_of_constraint'])
        ...
        for path in last_training_paths:
            ...
            path_l_returns = path_l_advs + path_l_values
        ...
        if training_diagnotic is not None:
            ...
            logger.logkv('violation_times', training_diagnotic['train-violation-average'])
            ...
            print('training_step:', global_step,
                  ...
                  'average violations:', training_diagnotic['train-violation-average'],
                  ...
                  *[(name,value) for (name, value) in zip(policy.diagnosis_names, mblossvals)])
        logger.dumpkvs()
        # 状态更新
