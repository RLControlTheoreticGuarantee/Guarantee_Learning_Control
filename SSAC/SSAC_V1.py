...
class SSAC(object):
    def __init__(self,
        ...
        gamma = variant['gamma']
        safety_gamma = variant['safety_gamma']
        ...
        self.cons_l = self._build_l(self.S, self.a, reuse=True)

        # lyapunov constraint

        self.l_derta = tf.reduce_mean(self.cons_l - self.safety_threshold)
        ...
        with tf.control_dependencies(target_update):  # soft replacement happened at here
            ...
            l_target = self.l_R + safety_gamma * (1-self.terminal)*l_   # Lyapunov critic - self.alpha * next_log_pis
    ...
    def learn(self, LR_A, LR_C, LR_L):
        ...
        feed_dict = {self.a_input: ba, self.S: bs, self.S_: bs_, self.R: br, self.l_R: blr, self.terminal: bterminal,
                     self.LR_C: LR_C, self.LR_A: LR_A, self.LR_L: LR_L, self.safety_threshold:self.threshold}
...
def train(variant):
    ...
    logger.logkv('safety_threshold', policy_params['threshold'])
    logger.logkv('safety_gamma', policy_params['safety_gamma'])
