...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            ...
            # l_rewards= min(abs(xposafter), 5)
            if np.size(rewards)==4:
                l_rewards = [infos[0]['l_rewards'], infos[1]['l_rewards'], infos[2]['l_rewards'], infos[3]['l_rewards']]
            else:
                l_rewards = [infos[0]['l_rewards']]
            # if RENDER==True:
            #     self.env.render()
            # print(infos)
            for info in infos:
