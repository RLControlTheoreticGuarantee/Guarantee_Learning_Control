...
try:
    ...
    MPI = None
from sppo.runner import Runner
...
def learn(*, network, env, total_timesteps,n_of_paths=10, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
            vf_coef=0.5,lf_coef=0.5, max_grad_norm=0.5, gamma=0.99, lam=0.95, safety_gamma=0.5, safety_threshold = 0.,
          init_labda = 1.,labda_clip_range = 0.8,
            log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2, use_adaptive_alpha3=True,approximate_value_function=True,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if model_fn is None:
        from baselines.sppo.model import Model
        model_fn = Model

    model = model_fn(policy=policy, ob_space=ob_space, ac_space=ac_space, nbatch_act=nenvs, nbatch_train=nbatch_train,
                    nsteps=nsteps, ent_coef=ent_coef, vf_coef=vf_coef,lf_coef=lf_coef, init_labda=init_labda,
                    max_grad_norm=max_grad_norm,  threshold=safety_threshold,)
    print("model build success")

    if load_path is not None:
        model.load(load_path)
    # Instantiate the runner object
    runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, safety_gamma = safety_gamma, lam=lam)
    ...
    logger.logkv('safety gamma', safety_gamma)
    logger.logkv('safety threshold', safety_threshold)
    ...
    for update in range(1, nupdates+1):
        ...
        if update % log_interval == 0 or update == 1:
            ...
            logger.logkv('eplrewmean', ep_L_R)
