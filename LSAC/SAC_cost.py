...
def train(variant):
    ...
    d_dim = disturber_params['disturbance_magnitude'].shape[0]
    a_upperbound = env.action_space.high
    a_lowerbound = env.action_space.low
    policy = SAC_cost(a_dim,s_dim,d_dim, policy_params)
    ...
    for i in range(max_episodes):
        ...
        for j in range(max_ep_steps):
            ...
            pool.store(s, a, np.zeros([d_dim]), np.zeros([d_dim]), r, terminal, s_)
