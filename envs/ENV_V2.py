...
class CartPoleEnv_adv(gym.Env):
    ...
    def set_params(self, length, mass_of_cart, mass_of_pole):
    ...
    def get_params(self):
        ...
        return self.length, self.masspole, self.masscart
    ...
    def step(self, action, impulse=0, process_noise=np.zeros([4])):
        a = 0
        ...
        if self.kinematics_integrator == 'euler':
            ...
            theta = theta + self.tau * theta_dot + process_noise[0]
            theta_dot = theta_dot + self.tau * thetaacc + process_noise[2]
            ...
            x = x + self.tau * x_dot + process_noise[1]
            x_dot = x_dot + self.tau * xacc + process_noise[3]
            # x_dot = np.clip(x_dot, -self.max_v, self.max_v)
